...
const editedCategory = async (req, res) => {
  try {
    ...
    const normalizedCategoryName = categoryName.toLoweCase()
    const existingCategory = await Category.findOne({
      deletedAt : 'listed',
      categoryName : {$regex : new RegExp(`^${normalizedCategoryName}$`,'i')},
      _id : {$ne : categoryId }
    })
...
const category  = await Category.findById(categoryId)
  ...
  await Product.updateMany({selectedCategory : category.categoryName},{ $set: { deletedAt: 'Deleted' } })
