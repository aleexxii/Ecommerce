...
const getHome = async (req, res) => {
  ...
  try {
    ...
    console.log("start",{ categories, product, cartCount, banners, topSellingCategories, bestSellingProducts });
...
const search = async (req, res) => {
  try {
    ...
    // Step 1: Find categories matching the search query
const matchedCategories = await Category.find({
  categoryName: { $regex: search, $options: 'i' },
  deletedAt: 'listed'
});

const categoryNames = matchedCategories.map(category => category.categoryName);

const searchedProducts = await Product.find({
  deletedAt: 'Not-Deleted',
  $or: [
    { productName: { $regex: search, $options: 'i' } },
    { selectedCategory: { $in: categoryNames } } // Match products with selected categories
  ]
});
